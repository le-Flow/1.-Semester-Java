...
import bundesliga.Table.TableType;
...
public class TableEntry implements Comparable<TableEntry>{
    ...
    public static TableEntry createTableEntry(League league, Team team, Integer matchDay, TableType type) {
        TableEntry tableEntry = new TableEntry(league, team, matchDay, type);

        for (Match match : league.getMatches(team)) {
            if (matchDay != null && match.getMatchDay() > matchDay) {
                continue;
            }

            if (type == TableType.HOME && match.getHomeTeam().equals(team)) {
                tableEntry.addMatch(match);
            } else if (type == TableType.AWAY && match.getAwayTeam().equals(team)) {
                tableEntry.addMatch(match);
            } else if (type == TableType.BOTH) {
                tableEntry.addMatch(match);
            }
        }
    ...
    public int compareTo(TableEntry other) {
        int result = this.points - other.points;
        if (result != 0) {
            return -result; // higher points should come first
        }

        result = this.goalDifference - other.goalDifference;
        if (result != 0) {
            return -result; // higher goal difference should come first
        }

        result = this.goalsFor - other.goalsFor;
        if (result != 0) {
            return -result; // higher goals for should come first
        }

        return this.team.getName().compareTo(other.team.getName()); // alphabetical order by team name
    }
